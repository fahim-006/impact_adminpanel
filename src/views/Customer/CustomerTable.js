...
import { deleteCustomerById, getcustomersById, updateCustomerById } from "src/api/apiCustomer";
...
const CustomersTable = ({item}) => {
    ...
    const handleShow = (id) =>{
        //alert(id)
        getcustomersById(id)
        .then(response => setcustomerbyid(response.data))
    ...
    const handleSubmit = (e) => {
        ...
        updateCustomerById(customerbyid, id)
            .then(response => {
    ...
    return (
      ...
      {redirect ? window.location.replace("/customer"): ""}
        <tr>
            <td>{item.name}</td>
            <td>{item.email}</td>
            <td>{item.phoneNo}</td>
