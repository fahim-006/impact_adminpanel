...
import { getAllGroup,getGroupById,deleteGroupById,updateGroupById } from "src/api/apiGroups";
...
const {
  ...
  useEffect(()=>{
     ...
     const handleShow = (id) =>{
      //alert(id)
      getGroupById(id)
      .then(response => setGroupbyid(response.data))
      .catch(err => alert("something went wrong"))
      setShow(true);
  }
  ...
  return(
      <>  
        ...
        <Modal.Header closeButton>
          <Modal.Title>Fill WireHouse Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {profileForm()}
        </Modal.Body>
      ...
      {redirect ? window.location.replace("/user_group/manage_group_user"): ""}
